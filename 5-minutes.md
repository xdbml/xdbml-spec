...
**xDBML** is a open text-based **markup language** for **describing the shape** of **structured and semi-structured data**: relational tables, document collections, event records, JSON columns, graph relationships, views, API contracts. 

One file, many targets. It is the markup that humans, AI assistants, and data modeling tools all use to describe the same schemas without translation loss.
...
- **Two namespace levels (Containers)**: one for an Oracle schema, one for a MongoDB database
...
This single file generates an Oracle DDL for `customers`, a MongoDB `$jsonSchema` validator for `orders`, a JSON Schema for an OpenAPI contract, an Avro schema for an event stream, and the schema section of an ODCS data contract.



## Why this exists

Every modern data platform mixes paradigms. A typical SaaS product stores users in Oracle, events in Kafka with Avro schemas, application state in MongoDB, analytics in BigQuery, Databricks, or Snowflake, and social graphs in Neo4j. Each technology has its own schema language.

**Hand-maintaining five schemas that describe the same business concepts is where mistakes live.** A field renamed in Oracle doesn't propagate to the MongoDB validator. A new payment method added to the Avro event schema isn't reflected in the Databricks warehouse table. Drift between schemas is one of the most common sources of production data bugs.
...
But more importantly: **xDBML is the markup that AI assistants and modeling tools use to describe schemas.** When you ask Claude, GPT-5, or any modern LLM to "design a schema for X," the model produces nested objects, polymorphic types, foreign-key references, and arrays of records. 

JSON Schema can express most of these but only for JSON-shaped data; SQL DDL can express tables and constraints but not nested types; Avro can express records but not relational schemas. 

xDBML is the only mainstream markup that expresses *all* of them, in the same syntax, with the AI-readiness metadata (descriptions, synonyms, business terms, tags, granularity) that lets natural-language queries resolve to canonical schema elements without guesswork.
...
Each storage traditionally has its own word for the same concepts. xDBML accepts all of them:
...
DBML's four cardinality operators express maximum cardinality. xDBML adds explicit min/max for further precision:
...
Entity customers {
  mrr_amount decimal [
    note= "Represents the total monthly recurring revenue attributed to the customer. This value reflects the normalized monthly income generated from active subscriptions or contracts, excluding one-time charges, discounts, and taxes. Typically used for revenue tracking, customer segmentation, and forecasting in subscription-based business models."
...
- **Read the [v0.1 specification](https://github.com/xdbml/xdbml-spec/tree/main/spec/v0.1/SPEC.md)** for the full language reference.
- **Try it in the [playground](https://playground.xdbml.org)** (TBA) — paste xDBML, see rendered diagrams and generated artifacts.
...
*Last updated May 17, 2026. Apache License 2.0. xdbml.org*
