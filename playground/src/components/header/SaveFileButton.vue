<template>
  <div ref="rootEl" class="relative inline-flex items-stretch">
    ...
    <button
      type="button"
      class="inline-flex items-center gap-1.5 pl-2.5 pr-2 py-1 text-xs font-medium text-gray-700 border border-transparent rounded-l transition-colors cursor-pointer hover:bg-gray-100 hover:border-gray-200"
      :class="{ 'text-blue-700 bg-blue-50 hover:bg-blue-100': isDirty }"
      :title="saveTitle"
      @click="onSave"
    >
    ...
    <button
      type="button"
      class="inline-flex items-center pl-1 pr-1.5 py-1 text-xs font-medium text-gray-500 border border-transparent rounded-r transition-colors cursor-pointer hover:bg-gray-100 hover:text-gray-700 hover:border-gray-200"
      :class="{ 'text-blue-600 bg-blue-50 hover:bg-blue-100': isDirty }"
    ...
    >
      <div
        v-if="isOpen"
        class="absolute top-full right-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50"
        role="menu"
      >
        <button
          type="button"
          class="w-full text-left px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 active:bg-gray-100 transition-colors rounded-t-lg flex items-center gap-2"
          role="menuitem"
          @click="onSaveAs"
        >
          <svg viewBox="0 0 16 16" class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            <path d="M3 2h8l3 3v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1z"/>
            <path d="M5 2v4h6V2"/>
          </svg>
          <span class="flex-1">Save as&hellip;</span>
          <span class="text-[10px] text-gray-400 font-mono">Ctrl+Shift+S</span>
        </button>
      </div>
    </Transition>
  </div>
