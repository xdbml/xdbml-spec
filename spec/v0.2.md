...
description: The complete xDBML v0.2 specification. Twenty-nine chapters plus five appendices covering lexical conventions, entities, relationships, polymorphism, views, AI-readiness metadata, validation constraints, the module system, AST representation, conformance, and DBML 3.13.6 compatibility.
---
...
**What's new in v0.2**: the module system (§25), allowing schemas to be split across multiple files via `use` and `reuse` directives, with optional inline clone blocks for file autonomy; and scalar Named Types (§13.7), extending Named Types to describe single-shape declarations rather than only object-shaped structures.
...
26. [AST representation](#26-ast-representation)
27. [Round-trip semantics](#27-round-trip-semantics)
28. [Relationship to other standards](#28-relationship-to-other-standards)
...
**Round-trip is lossy by design** with respect to target-specific features. An xDBML schema lifted from Oracle DDL preserves the logical content (tables, columns, types, declarative constraints, relationships, indexes in declarative form). It does not preserve operational specifics (partitions, storage configuration, triggers, PL/SQL) -- those are deliberately outside xDBML's scope and remain in the modeling tool's native representation.
...
### 13.7 Scalar named types (new in v0.2)

The Type declarations in §13.1 through §13.6 describe object-shaped reusable structures -- a Type body contains one or more named inner fields. v0.2 extends Named Types to also support **scalar shapes** -- a Type whose entire content is a single field's worth of declaration.
...
**Importability**: scalar Named Types are importable via the `type` element type in the module system (§25):
...
**Relationship to field-level imports**: scalar Named Types and field-level imports (§25.9) both follow the declaration-vs-placement pattern: the construct declares a reusable shape; placement happens by using the shape's name as a field's type. The difference is in where the reusable shape originates: scalar Named Types are explicitly declared as `Type`; field-level imports clone a shape from an existing entity field. Both mechanisms are valid; authors choose based on context.
...
### 25.2 Import in full vs selective import
...
A single directive may import multiple declarations from the same source file. When combined with a clone block (§25.6), the clone block contains all the imported declarations matched by name and element type.

### 25.3 Importable element types
...
| `type` | Named Type (object-shaped or scalar -- §13) |
| `edge` | Edge |
| `view` | View |
| `diagramview` | DiagramView |
| `field` | Single field declaration cloned from an entity as a reusable named shape (see §25.8) |
...
### 25.4 The `use` and `reuse` directives
...
### 25.5 Where the directive goes determines where the import lives
...
**For field imports**, the directive MUST appear at file scope (see §25.8 for the declaration-vs-placement pattern).
...
### 25.7 Worked example: `use` vs `reuse`
...
A field cannot exist on its own outside an entity. So the field import directive declares an **available named field shape** at file scope, and the author then uses that name as the type of a field in some entity. This makes field imports semantically similar to scalar Named Types (§13.7): the directive declares a reusable shape; the author places fields of that shape elsewhere in the file.
...
### 25.11 Version directive across files
...
### 25.16 Parser implementation requirements
...
## 27. Round-trip semantics

### 27.1 Round-trip targets
...
## 28. Relationship to other standards
...
1. **Parses every valid xDBML v0.2 document** to the AST described in §26.
...
8. **Computes default cardinality** per §10.8 when not declared.
9. **Implements the module system** (§25) including relative-path imports, clone blocks, field-level imports, and scoped imports per §25.6.
