...
| `array` / `set` | one element type | single type -- no separator |

An `array`/`set` takes a single element *type expression*; a mix of element types is expressed by making that expression a `union` (scalars) or `oneOf` (object shapes), or by using a tuple (fixed positions) -- not by listing types in the brackets (§8.4, §8.6, §20).
...
The brackets after `array`/`set` hold a single element *type expression*, not a comma-separated list. The common case is a uniform element type:
...
JSON and BSON arrays are not required to be uniform. xDBML models mixed elements by making the element type itself polymorphic rather than by listing types in the brackets: use a `union` for a mix of scalar types (union members are scalars or `null`, §20.1), `oneOf` for a mix of object shapes (§20.5), or a tuple for a fixed positional layout (§8.6). For a value with no element typing at all, use an opaque `json` field (§21.1).

```xdbml
ids     array [union [int, varchar]]  // each element is an int OR a varchar
```
