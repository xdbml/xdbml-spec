...
At most one `Project` may appear. Recognized settings: `targets`, `Note`.

### 5.1 Target name vocabulary

Target names are bare identifiers when they have no spaces or special characters (`Oracle`, `PostgreSQL`, `MongoDB`, `Snowflake`, `Neo4j`, `Avro`, `Cassandra`, `BigQuery`, `Databricks`). They must be quoted when they contain spaces or punctuation (`'SQL Server'`, `'Cosmos DB'`, `'JSON Schema'`, `'AWS DocumentDB'`).

A non-exhaustive list of recognized target names: `Oracle`, `PostgreSQL`, `'SQL Server'`, `MySQL`, `MariaDB`, `SQLite`, `Snowflake`, `Databricks`, `BigQuery`, `Redshift`, `'Synapse Analytics'`, `MongoDB`, `'AWS DocumentDB'`, `'Cosmos DB'`, `Couchbase`, `RavenDB`, `DynamoDB`, `Cassandra`, `ScyllaDB`, `Neo4j`, `'Memgraph'`, `Neptune`, `JanusGraph`, `Avro`, `Parquet`, `'Protocol Buffers'`, `'JSON Schema'`, `OpenAPI`, `GraphQL`, `Elasticsearch`, `OpenSearch`, `TimescaleDB`, `ClickHouse`. Values outside this list are accepted but may produce generic output from target-aware generators.
