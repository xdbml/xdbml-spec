...
outline: [2, 4]
description: The complete xDBML v0.4 specification. Thirty chapters plus five appendices covering lexical conventions, entities, relationships (referential and denormalized replication), polymorphism, views, AI-readiness metadata, validation constraints, entity-level checks, the module system (including remote module sources), AST representation, conformance, and DBML 3.13.6 compatibility.
---
...
**Strict-superset claim**: every valid DBML document parses correctly under xDBML rules, and every DBML construct (used in a way valid in DBML) means the same thing in xDBML as in DBML. When a file declares `xdbml: 0.4` (or any 0.x), it has opted into xDBML's extended semantics; the semantic meaning of constructs in that context may legitimately differ from DBML's, because the file is no longer claiming to be DBML.

**What's new in v0.4**: the foreign master relationship type (§11.10). A `Ref` may now carry a `foreign_master` flag that records where a duplicated attribute of denormalized data is mastered, alongside the referential relationships -- foreign keys -- that xDBML has expressed since v0.1. A foreign master relationship documents duplication rather than structure: it reaches no DDL, it names a single attribute on each side, and a child attribute has at most one master (§11.11). §11.12 defines the four attribute role markers that an implementation computes from the declared relationships, and §11.13 gives the line style a renderer draws for each relationship type. Relationships also gain the properties that conceptual and logical modeling need, and that every mainstream ER tool exchanges: roles and verbs on each end (§11.14), the identifying or non-identifying constraint type (§11.15), and endpoints that name an entity rather than an attribute, so a relationship can be drawn before either entity has attributes and then refined in place (§11.16). A settings block is now accepted in the long `Ref` form as well (§11.2). This is a strict, backward-compatible addition: every v0.3 document remains valid under v0.4. (v0.3 introduced remote module sources (§26.14); v0.2 introduced the module system itself (§26) and scalar Named Types (§14.7).)
...
### 3.8 Element and field separators
...
An `array`/`set` takes an element *type expression*. For a mix of element types, use a `union` (scalars/named types) or `oneOf` (object shapes), or a tuple (fixed positions); for `array` and `set`, a comma-separated list of scalar/named types is shorthand for a `union` (§8.4, §8.6, §20).
...
| `xdbml: 0.4` | 0.3 | Warning + refuse to parse. |
...
JSON and BSON arrays are not required to be uniform. For a mix of scalar types, list them comma-separated: `array [T1, T2, ...]` is shorthand for `array [union [T1, T2, ...]]` (§20.1), so these two are equivalent:
...
The comma-list shorthand works the same for `array`, `list`, and `set`, and its members must be scalar or named types (or `null`) -- the members a `union` accepts. For a mix of object shapes use `oneOf` (§20.5); for a fixed positional layout use a tuple (§8.6); for a value with no element typing at all use an opaque `json` field (§21.1).
...
A relationship whose endpoints carry attributes of their own is an Edge (§12) rather than a `Ref`, and that difference stays visible in the syntax rather than being inferred from whether attributes happen to be present.
...
| `<>` | Rejected. Many-to-many states a cardinality, which is unstated here, and a many-to-many relationship that carries its own attributes is an Edge (§12) |

`undirected: true` marks a relationship that reads the same way from both ends, such as a phone and a headset that are paired with each other. That differs from `<>`, which says many on both sides, and from `-`, which says nothing about direction at all. The setting matches `undirected` on an Edge (§12.3).
...
**Lossless:** Container declarations and settings; Entity, View, Edge, Field, Index, Ref structure including relationship type, roles, verbs, constraint type, and entity-level endpoints; nested types; polymorphism; named types (where target supports `$ref`); validation constraints; cardinality; notes.
...
- Foreign master relationships to DBML and to every engine DDL target (dropped; they describe duplication in the data rather than structure in the database, and no target expresses them)
...
A conforming xDBML v0.4 implementation:

1. **Parses every valid xDBML v0.4 document** to the AST described in §27.
2. **Parses every valid xDBML v0.3 document** with v0.3 semantics (the version directive selects).
...
13. **Implements the module system** (§26) including relative-path imports, clone blocks, field-level imports, and scoped imports per §26.6, and remote module sources over HTTPS per §26.14 (the `https`-only scheme restriction, and resolution of relative references against a remote module's base URL per §26.14.1).
...
6. xDBML extension constructs are not recognized in documents without a version declaration.
7. DBML's implicit-schema syntax (`Table core.users { ... }`) remains valid; produces an implicit Container.
8. DBML's `Table` keyword remains valid as a synonym for `Entity`.
9. DBML's `use` and `reuse` module-system directives (without clone blocks, without xDBML-specific element types) behave identically in xDBML. v0.2 adds clone blocks, field-level imports, the extended element-type vocabulary, and Container-scoped imports; v0.3 adds remote module sources over HTTPS -- all available only when the file declares the corresponding `xdbml:` version.
10. DBML has no counterpart to the foreign master relationship, to relationship roles, verbs and constraint type, or to entity-level relationship endpoints. A generator writing DBML drops the documentation settings and omits an entity-level relationship, which has no endpoints a DBML `Ref` can name.
11. DBML has no counterpart to the foreign master relationship. A generator writing DBML omits the relationship rather than emitting it as a `Ref`, because a DBML `Ref` is a foreign key and a downstream tool would read it as enforceable structure. An omitted relationship may be recorded as a comment.

A DBML 3.13.6 document is upgraded to xDBML by adding `xdbml: 0.4` at the top (or an earlier `xdbml:` version for an earlier feature set). No structural changes required.
...
*End of xDBML Specification v0.4.*
